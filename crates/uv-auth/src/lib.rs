...
///
/// Respects `UV_AUTH_URLS` with whitespace separated URLs.
///
...
/// - `UV_AUTH_URLS="username:password@hostname/path"`
///
...
fn parse_url_from_env(url: &str) -> Option<Url> {
    ...
    .inspect_err(|err| debug!("Ignoring invalid URL in `UV_AUTH_URLS`: {err}"))
    .ok()
}
