...
impl Lock {
    ...
    pub fn from_resolution_graph(graph: &ResolutionGraph) -> Result<Self, LockError> {
        let requires_python = graph.requires_python.as_ref();
        ...
        for node_index in graph.petgraph.node_indices() {
            ...
            if dist.is_base() {
                ...
                for edge in graph.petgraph.edges(node_index) {
                    ...
                    locked_dist.add_dependency(requires_python, dependency_dist, marker);
                }
    ...
    pub fn with_supported_environments(mut self, supported_environments: Vec<MarkerTree>) -> Self {
        ...
        self.supported_environments = supported_environments
            .into_iter()
            .map(|marker| {
                let Some(ref requires_python) = self.requires_python else {
                    return marker;
                };
                requires_python.complexify_markers(marker)
            })
            .collect();
        self
    }
    ...
    pub fn simplified_supported_environments(&self) -> Vec<MarkerTree> {
        self.supported_environments()
            ...
            .map(|marker| {
                ...
                requires_python.simplify_markers(marker.clone())
            })
            .collect()
    }
    ...
    pub fn to_toml(&self) -> anyhow::Result<String> {
        ...
        if !self.fork_markers.is_empty() {
            let fork_markers = each_element_on_its_line_array(
                self.fork_markers
                    .iter()
                    .map(|marker| {
                        SimplifiedMarkerTree::new(self.requires_python.as_ref(), marker.clone())
                    })
        ...
        for dist in &self.packages {
            packages.push(dist.to_toml(self.requires_python.as_ref(), &dist_count_by_name)?);
        }
...
impl TryFrom<LockWire> for Lock {
    ...
    fn try_from(wire: LockWire) -> Result<Lock, LockError> {
        ...
        let packages = wire
            .packages
            .into_iter()
            .map(|dist| dist.unwire(wire.requires_python.as_ref(), &unambiguous_package_ids))
            .collect::<Result<Vec<_>, _>>()?;
        let supported_environments = wire
            .supported_environments
            .into_iter()
            .map(|simplified_marker| simplified_marker.into_marker(wire.requires_python.as_ref()))
            .collect();
...
impl Package {
    ...
    fn add_dependency(
        ...
        let new_dep = Dependency::from_annotated_dist(requires_python, annotated_dist, marker);
    ...
    fn add_optional_dependency(
        ...
        let dep = Dependency::from_annotated_dist(requires_python, annotated_dist, marker);
...
impl Dependency {
    ...
    fn from_annotated_dist(
        ...
        Dependency::new(requires_python, package_id, extra, complexified_marker)
    }

    /// Returns the TOML representation of this dependency.
    fn to_toml(
        &self,
        _requires_python: Option<&RequiresPython>,
        dist_count_by_name: &FxHashMap<PackageName, u64>,
    ) -> Table {
        let mut table = Table::new();
        self.package_id
