...
fn lock_redact_https() -> Result<()> {
    ...
    // Installing with credentials from with `UV_AUTH_URLS` should succeed.
    uv_snapshot!(context.filters(), context.sync().arg("--frozen").arg("--reinstall").arg("--no-cache").env("UV_AUTH_URLS", "public:heron@pypi-proxy.fly.dev"), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
